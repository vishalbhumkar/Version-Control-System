...
const Copilot = () => {
  ...
  const fetchAllThreads = async () => {
    try {
      const response = await axios.get("http://localhost:3001/copilot/thread");
  ...
  const selectThread = async (threadId) => {
    ...
    setShowHistory(false);
    try {
      const response = await axios.get(`http://localhost:3001/copilot/thread/${threadId}`);
  ...
  const deleteThread = async (e, threadId) => {
    e.stopPropagation();
    try {
      await axios.delete(`http://localhost:3001/copilot/thread/${threadId}`);
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:3001/copilot/chat", {
